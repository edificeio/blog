...
export function needRedirect(): string | undefined {
  const hasLocation = location.hash.substring(1);

  // Check if the URL is an old format (angular root with hash) and redirect to the new format
  if (hasLocation) {
    const isBlogPath = matchPath("/view/:blogId", hasLocation);
    if (isBlogPath) {
      // Redirect to the new format
      return `/id/${isBlogPath?.params.blogId}`;
    }

    const isPostPath = matchPath("/detail/:blogId/:postId", hasLocation);
    if (isPostPath) {
      // Redirect to the new format
