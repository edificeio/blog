...
export function LibraryDelegate(
  $scope: LibraryControllerScope,
  $rootScope,
  $location
) {
  $scope.displayLib = {
  ...
  $scope.hasFiltersActive = () => {
    return (
      $scope.filters.mine && $scope.filters.public && $scope.filters.shared
    );
  };
  ...
  $scope.printBlog = function (blog: Blog, printComments: boolean) {
    if (blog) {
      window.open(
        `/blog/print/blog#/print/${blog._id}?comments=${printComments}`,
        "_blank"
      );
    }
  };
  ...
  $scope.can = (right: string) => {
    ...
    return (
      folder.ressources.sel.selected.find((w: Blog) => !w.myRights[right]) ===
      undefined
    );
  };
  ...
  $scope.tryTrashSelection = async () => {
    const founded = $scope.currentFolder.selection
      .filter((f) => f instanceof Blog)
      .map((f) => f as Blog)
      .findIndex((f) => f.visibility == "PUBLIC");
    if (founded > -1) {
  ...
  $scope.lightbox = function (lightboxName: string, data: any) {
    ...
    $scope.displayLib.lightbox[lightboxName] =
      !$scope.displayLib.lightbox[lightboxName];
  };
  ...
  $scope.showEmptyScreenSearch = () => {
    ...
    return (
      ...
      ($scope.currentFolder.ressources.all.length ||
        $scope.currentFolder.children.all.length) &&
      $scope.currentFolder.children.all.filter($scope.searchFolder).length ==
        0 &&
  ...
  $scope.showEmptyScreenFilter = () => {
    ...
    return (
      !(
        ...
        $scope.currentFolder.children.all.filter($scope.searchFolder).length ==
          0 &&
        $scope.currentFolder.ressources.all.filter($scope.searchBlog).length ==
          0
      ) &&
      !$scope.hasFiltersActive() &&
